...
public class SeckillOrderController {
    ...
    @GetMapping("/add")
    public Result orderSeckillGoods(Long id, String time, String username) {
        try {
            /* String username = "szitheima";*/
            Boolean status = seckillOrderService.orderSeckillGoods(username, id, time);
            if (status) {
                return new Result(true, StatusCode.OK, "进入排队中...");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return new Result(false, StatusCode.NOTFOUNDERROR, "秒杀失败");
    }
